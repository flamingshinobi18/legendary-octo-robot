	...
	def greet(Hindi_Text):
		...
		for j in range(len(test_input)):
			...
			for i, x in enumerate(pos_tags):
				...
				d[test_words[i]]=tags_desc[tags[x]]
		
		op=""
		for k,v in d.items():
			op+=k+" -> "+v+'\n'+'\n'
		return op
	# outputs = [gr.Dataframe(row_count = (len(test_words), "dynamic"), col_count=(len(test_words), "fixed"), label="Predictions", headers=["Failures"])]
 
	demo = gr.Interface(fn=greet, title="POS tagger for Hindi",inputs="text", outputs="text")
